...
@MainActor
final class SKKEngineSpec : QuickSpec, SKKDelegate, Sendable {
    // delegate
        ...
        MainActor.assumeIsolated {
            ...
            context("hirakana mode") {
                describe("ひらかな input") {
                    ...
                    it("convert kanji with okuri") { @MainActor in
                    ...
                    context("with dakuten") {
                        it("convert kanji on compose mode") { @MainActor in
                        ...
                        it("convert kanji on register mode") { @MainActor in
                        ...
                        it("convert kanji without dakuten") { @MainActor in
                            engine.handle(.char(kana: "わ", shift: true))
                    ...
                    it("toggle dakuten") { @MainActor in
                ...
                describe("number input") {
                    ...
                    it("ignore okuri for number") { @MainActor in
                        engine.handle(.char(kana: "1", shift: true))
                ...
                describe("alphabet input") {
                    ...
                    it("ignore okuri for alphabet") { @MainActor in
                        engine.handle(.char(kana: "a", shift: true))
                        engine.handle(.char(kana: "b", shift: true))
                        engine.handle(.enter)
                        expect(self.insertedText).to(equal("b"))
                    }
                    it("toggle upper/lower") { @MainActor in
                        engine.handle(.char(kana: "a", shift: false))
                        engine.handle(.char(kana: "b", shift: false))
                ...
                describe("「っ」送り仮名変換") {
                    it("can convert 「はいった」") { @MainActor in
                    ...
                    it("can convert 「はいっ」") { @MainActor in
                        engine.handle(.char(kana: "は", shift: true))
                        engine.handle(.char(kana: "い", shift: false))
                    ...
                    it("can convert 「ひっぱる」"){ @MainActor in
                    ...
                    it("can convert 「ばっする」"){ @MainActor in
                        engine.handle(.char(kana: "は", shift: true))
                        engine.handle(.toggleDakuten(beforeText: ""))
                        engine.handle(.char(kana: "つ", shift: false))
                        engine.handle(.toggleDakuten(beforeText: ""))
                ...
                describe("dictionary") {
                    it("can register dakuten kana") { @MainActor in
                        engine.handle(.char(kana: "か", shift: true))
                        engine.handle(.char(kana: "か", shift: false))
                        engine.handle(.char(kana: "か", shift: false))
                        engine.handle(.space)
                    ...
                    it("can register word with okuri") { @MainActor in
                        // かかk を辞書に登録する
                        engine.handle(.char(kana: "か", shift: true))
                        engine.handle(.char(kana: "か", shift: false))
                        engine.handle(.char(kana: "か", shift: true))
