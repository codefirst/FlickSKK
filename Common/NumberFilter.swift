...
class NumberFilter : SKKFilter {
    ...
    private let regexp : NSRegularExpression! = NSRegularExpression(pattern: "[0-9]+", options: nil, error: nil)

    // 単語中の #1 等を置換するための正規表現(e.g. #1, #2)
    private let template : NSRegularExpression! = NSRegularExpression(pattern: "#[0-9]", options: nil, error: nil)
    ...
    private func hashnize(target : String) -> String {
        return regexp.stringByReplacingMatchesInString(target,
            options: nil,
            range: NSMakeRange(0, count(target.utf16)),
            withTemplate: "#")
    }
